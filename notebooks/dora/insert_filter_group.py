...
# ── 6. filter_values_unity — CTFC project_url (Cycle Time for Changes) ────────
# CTFC joins pipeline_deployment_commits via pipeline_activities.pipeline_commit_sha
# The filter only needs project_url — no deployment_stages or pipeline_status_success
FVU_CTFC_ID = str(uuid.uuid4())
spark.sql(f"""
    INSERT INTO {CATALOG}.master_data.filter_values_unity
        (id, filter_group_id, tool_type, filter_name, filter_values, kpi_uuids,
         custom_fieldName, created_by, created_at, updated_by, updated_at,
